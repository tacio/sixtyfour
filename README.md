...
Name the **shape, then the texture**. Four texture words cover every tier:
...
The texture words are listed in ink order rather than value order, because that is the order they read as a ramp.

## It degrades to itself

Nothing here is an image, an icon set, or a substitution. The characters underneath are ordinary base64 — the font only changes how they are drawn:

```html
<span class="sixty-four">dGhlIHRleHQgaXMgc3RpbGwgYmFzZTY0</span>
```

Take the font away and that is still `dGhlIHRleHQgaXMgc3RpbGwgYmFzZTY0`. Which means, for free:

- **Select and copy** puts the base64 on your clipboard, not a row of pictures
- **Ctrl+F**, your editor, your terminal and `grep` all search the real characters
- **Screen readers** announce the string itself — no alt text to write and none to get wrong
- **Diffs, logs, `curl`, a plain text field, a tool that has never heard of this font** — the bytes never changed

The font is a display layer and nothing else. That is the argument for doing this as a typeface rather than as sixty-four SVGs.
...
| `mapping.json` / `mapping.tsv` | The alphabet as data — including how each symbol is said out loud — for writing your own renderer. |
| `specimen.html` | Interactive specimen: live encoder, the full matrix, the spoken names, a section set entirely in the colourless face, a graceful-degradation demo, a Light / Dark / Auto control, a both-grounds panel, and a switch that drops the colour so you can check the hatch on its own. |
| `specimen.png` / `grounds.png` | The full set, and the light/dark comparison above. |
...
uv run pytest                # 57 checks, including the hatch ink ramp
```
